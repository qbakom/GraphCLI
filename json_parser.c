...
// Global variables to store parsed graph data
int parsed_nodes = 0;
int parsed_edges[100][2];
int edge_count = 0;

// Buffer to collect JSON response chunks
...
int parseAndPrintResponse(char *response, int size, int n) {
    ...
    // We'll parse after all chunks are received
    return real_size;
}
...
// This function should be called after curl_easy_perform to process the collected data
void finishJsonParsing() {
    parseResponseBuffer();
    // Reset buffer position for next request
    buffer_pos = 0;
    response_buffer[0] = '\0';
}
